...
class Unittest_kmeans:
    ...
    #Checking shape of Centroid array output with sample input data
    ...
    #Checking shape of cluster dataframe output with sample input data
    ...
    #Checking shape of outliers dataframe output with sample input data
    ...
    #initializing synthetic data to run code and check if clustering label is as expected
    ...
    #Asserting if output cluster label is equal to expected label

    assert np.array_equal(np.array(clus["label"]) + 1, np.array(clus["cluster_label"])) or np.array_equal(np.array(clus["label"]), np.absolute(np.array(clus["cluster_label"]) - 2)), "Clustering NOT as predicted."
